...
export default function DekanSozlamalarPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            {loading ? (
                <div className={`rounded-2xl border p-5 sm:p-6 ${ui.card}`}>
                    <SkelForm fields={6} />
                </div>
            ) : loadError || !settings ? (
                        ...
                        {renderSection(Phone, 'Aloqa va favqulodda xizmatlar', 0.1, (
                          <>
                            <div className={`rounded-lg border p-3 text-[11px] leading-relaxed ${isLight ? 'border-indigo-200 bg-indigo-50 text-indigo-800' : 'border-indigo-500/25 bg-indigo-500/10 text-indigo-200'}`}>
                                <span className="font-black">Talaba kengashi raisi</span> endi bu yerda emas — u haqiqiy
                                rolga aylantirildi. Talabalar sahifasida kerakli talabani tanlab, &laquo;⋮&raquo; menyudan{' '}
                                <span className="font-black">&laquo;Kengash raisi tayinlash&raquo;</span>ni bosing.
                            </div>

                ...
                <div className={`relative backdrop-blur-2xl border rounded-[2.5rem] p-6 sm:p-10 shadow-3xl overflow-hidden ${isLight ? 'bg-white/80 border-slate-200' : 'bg-[#0b1120]/70 border-white/10'}`}>
                    ...
                    <form onSubmit={handleUpdate} className="space-y-5 sm:space-y-6">
                        ...
                        <motion.button
                            ...
                            disabled={loading}
                            className={`w-full h-14 sm:h-16 rounded-2xl font-black text-[10px] sm:text-[11px] tracking-[0.3em] uppercase text-white flex items-center justify-center gap-3 relative overflow-hidden transition-all ${loading ? isLight ? 'bg-slate-200 text-slate-400' : 'bg-white/5 text-slate-600' : isLight ? 'bg-blue-600 hover:bg-blue-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                        >
                            {loading ? (

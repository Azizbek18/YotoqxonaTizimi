  ...
  return (
    <div className={`flex justify-between items-center gap-2 ${divider ? 'pt-2.5 border-t border-white/5' : ''}`}>
      <div className="min-w-0">
        ...
        <p className={`text-[10px] ${t.textMuted}`}>{name}</p>
      </div>
      {phone ? (
        <a
          href={`tel:${phone}`}
          className={`shrink-0 flex items-center gap-1 px-2.5 sm:px-3 py-1.5 rounded-xl border text-[10px] font-bold ${
...
export default function SupportContactsCard({ isLight, contacts, settingsStatus, onRetry }: Props) {
  ...
  return (
    <div className={`backdrop-blur-xl border rounded-3xl sm:rounded-[32px] p-4 sm:p-6 ${t.surfaceBg}`}>
      <h3 className={`text-[10px] font-extrabold leading-relaxed mb-4 ${isLight ? 'text-blue-600' : 'text-cyan-400'}`}>

...
/** Superadmin section-management input (dorm_assign_section / dorm_clear_section). */
function parseSectionRef(input: unknown): { dormId: string; block: string; floor: number } {
  const s = (input ?? {}) as Record<string, unknown>
  const dormId = typeof s.dormId === 'string' ? s.dormId.trim() : ''
  if (!dormId) throw new ApiError(400, 'Yotoqxona tanlanmagan')
  const block = typeof s.block === 'string' ? s.block.trim().toUpperCase() : ''
  if (!/^[A-Z]$/.test(block)) throw new ApiError(400, 'Blok noto‘g‘ri')
  const floor = Number(s.floor)
  if (!Number.isInteger(floor) || floor < 1) throw new ApiError(400, 'Qavat noto‘g‘ri')
  return { dormId, block, floor }
}

// dorm_assign_section / dorm_clear_section raise P0003 when a section still
// holds residents from another faculty; P0001 when the dorm isn't blocked.
function mapSectionRpcError(error: unknown): never {
  const code = (error as { code?: string } | null)?.code
  if (code === 'P0003') {
    throw new ApiError(409, 'Bu seksiyada boshqa fakultetning talabalari bor — avval ularni ko‘chiring')
  }
  if (code === 'P0001') {
    throw new ApiError(409, 'Bu yotoqxona bloklarga bo‘linmagan')
  }
  if (code === 'P0002') {
...
export function createDormService(repository: DormRepository = createDormRepository()) {
  ...
  const service = {
    ...
    async create(input: unknown) {
      ...
      // A "blocked" building (A/B wings, per-section faculty ownership,
      // per-floor room numbering — 6-yotoqxona). Needs >= 2 blocks; the
      // room layout is then generated by dorm_build_blocked_layout, not the
      // free-form builder.
      const layoutKind = s.layoutKind === 'blocked' ? 'blocked' as const : 'simple' as const
      let blockCount = 1
      if (layoutKind === 'blocked') {
        blockCount = Number(s.blockCount ?? 2)
        if (!Number.isInteger(blockCount) || blockCount < 2 || blockCount > 8) {
          throw new ApiError(400, 'Bloklar soni 2–8 orasida bo‘lishi kerak')
        }
      }
    ...
    // ---- blocked-layout dorm sections (6-yotoqxona), superadmin only ----

    /** The full A1…B{floorCount} ownership grid — every cell, assigned or not. */
    async blockedGrid(dormId: string): Promise<BlockedDormGrid> {
      ...
      const dorm = await repository.getDorm(dormId)
      const [sections, residents] = await Promise.all([
        repository.listSections(dormId),
        repository.sectionResidentCounts(dormId),
      ])
      const byKey = new Map(sections.map((s) => [`${s.block}-${s.floor_number}`, s]))

      const cells: DormSection[] = []
      for (let b = 0; b < layout.blockCount; b += 1) {
        const block = String.fromCharCode(65 + b)
        for (let floor = 1; floor <= layout.floorCount; floor += 1) {
          ...
          cells.push({
            ...
            residentCount: residents.get(key) ?? 0,
          })
        }
      }
      ...
      return {
        ...
        roomsPerSection: 9,
        bedsPerSection: BLOCKED_BEDS_PER_SECTION,
        sections: cells,
      }
    },

    /** Generate the fixed 9-room-per-section template. Idempotent — safe to
     *  re-run after adding floors or fixing a mistake. */
    async buildBlockedLayout(dormId: string): Promise<{ created: number }> {
      const layout = await repository.getDormLayout(dormId)
      if (!layout) throw new ApiError(404, 'Yotoqxona topilmadi')
      if (layout.layoutKind !== 'blocked') {
        throw new ApiError(409, 'Bu yotoqxona bloklarga bo‘linmagan')
      ...
      try {
        const { created } = await repository.buildBlockedLayout(dormId)
        return { created }
      } catch (error) {
    ...
    async assignSection(input: unknown, staffId: string) {
      const { dormId, block, floor } = parseSectionRef(input)
      ...
      try {
        return await repository.assignSection(dormId, block, floor, faculty, staffId)
      } catch (error) {
        mapSectionRpcError(error)
      }
    },

    async clearSection(input: unknown) {
      const { dormId, block, floor } = parseSectionRef(input)
      try {
        return await repository.clearSection(dormId, block, floor)
      } catch (error) {
        mapSectionRpcError(error)
      }
    },

-- Keep the AI review decision in the same transaction that reserves the
-- receipt transaction (when present) and inserts every monthly payment row.
-- Manual-review submissions intentionally have no AI-read transaction id.

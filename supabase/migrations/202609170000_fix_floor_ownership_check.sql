-- ==========================================================
-- P3 tuzatish — qavat egaligi tekshiruvi + trigger search_path
-- ==========================================================
-- smoke-shared-dorm.mjs topgan xato: qavat FAQAT taklif qilingan
-- (dorm_floor.faculty = NULL, pending_faculty to'ldirilgan) bo'lsa,
-- `v_floor_owner IS NOT NULL AND v_floor_owner <> p_faculty` FALSE
-- bo'lib, taklif qiluvchi fakultet o'sha qavatga tarx chiza olardi /
-- talaba joylashtira olardi. To'g'ri qoida: qavat qatori BOR bo'lsa,
-- uning TASDIQLANGAN egasi aynan p_faculty bo'lishi shart
-- (`FOUND AND v_floor_owner IS DISTINCT FROM p_faculty` -> RAISE).
-- Qator umuman yo'q bo'lsa (bo'linmagan bino) — ruxsat (o'tish davri).
--
-- Bonus: 3 ta eski SECURITY DEFINER trigger funksiyasiga
...
DECLARE
  ...
  v_frozen boolean; v_occupied int; v_conflicting int;
BEGIN
  ...
  IF v_occupied >= p_max_capacity THEN RAISE EXCEPTION 'Room is full' USING ERRCODE = 'P0001'; END IF;
...
-- ----------------------------------------------------------
-- Trigger funksiyalariga search_path (advisor 0011)
-- ----------------------------------------------------------
ALTER FUNCTION public.check_student_permit_approved() SET search_path = public;
ALTER FUNCTION public.update_warning_count() SET search_path = public;
ALTER FUNCTION public.set_elonlar_updated_at() SET search_path = public;

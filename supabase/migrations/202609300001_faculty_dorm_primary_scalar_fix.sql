-- ==========================================================
-- Room RPClar: faculty_dorm skalyar so'rovlariga is_primary filtri
-- ==========================================================
-- 202609300000 faculty_dorm PK'ini (faculty) dan (faculty, dorm_id) ga
-- ko'tardi — bir fakultet endi bir nechta qatorga ega bo'lishi mumkin. Lekin
-- 5 ta RPC hali ham `SELECT dorm_id FROM faculty_dorm WHERE faculty = X` ni
-- SKALYAR ifoda sifatida ishlatadi (LIMIT yo'q, is_primary filtri yo'q) —
-- fakultetда 2-chi qator paydo bo'lgan zahoti bular
-- "more than one row returned by a subquery used as an expression" xatosi
-- bilan yiqiladi (xona biriktirish, tasdiqlash, layout saqlash — barchasi).
--
-- Bu migratsiya har bir shunday so'rovga `AND is_primary` qo'shadi — xuddi
-- ilova qatlamidagi (features/*/server/repository.ts) naqsh bilan bir xil.
-- Funksiya tanalari joriy prod holatidan (20260904001002 / 20260902083930)
-- olindi; yagona o'zgarish — is_primary filtri.
...
BEGIN
  ...
  v_dorm_id := (SELECT dorm_id FROM public.faculty_dorm WHERE faculty = v_faculty AND is_primary);
  IF v_dorm_id IS NULL THEN RAISE EXCEPTION 'No dorm for faculty %', v_faculty USING ERRCODE = 'P0002'; END IF;
...
-- ---------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.apply_building_layout(p_faculty text, p_numbering text, p_floors jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'

-- ==========================================================
-- replace_floor_room_layout / apply_building_layout: qaysi binoni
-- tahrirlashni aniq belgilash imkoni (p_dorm_id)
-- ==========================================================
-- Ikkalasi ham hozircha fakultetning ATIGI is_primary binosini topib
-- ishlaydi. Endi fakultet ikkinchi binoga ega bo'lishi mumkin (many-to-many,
-- 202609300000) va shu ikkinchi binoning tarxini chizish/generatsiya qilish
-- kerak bo'ladi — buning uchun ixtiyoriy `p_dorm_id` parametri qo'shiladi:
-- berilsa (va chindan ham shu fakultetga tegishli bo'lsa) o'sha bino
-- ishlatiladi, aks holda avvalgidek is_primary'ga tushadi (mavjud
-- chaqiruvlar o'zgarishsiz qoladi).

DROP FUNCTION IF EXISTS public.replace_floor_room_layout(text, integer, jsonb);

CREATE FUNCTION public.replace_floor_room_layout(
  p_faculty text, p_floor_number integer, p_rows jsonb, p_dorm_id uuid DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.replace_floor_room_layout(text, integer, jsonb, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.replace_floor_room_layout(text, integer, jsonb, uuid) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.replace_floor_room_layout(text, integer, jsonb, uuid) TO service_role;

-- ---------------------------------------------------------------------

DROP FUNCTION IF EXISTS public.apply_building_layout(text, text, jsonb);

CREATE FUNCTION public.apply_building_layout(
  p_faculty text, p_numbering text, p_floors jsonb, p_dorm_id uuid DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.apply_building_layout(text, text, jsonb, uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.apply_building_layout(text, text, jsonb, uuid) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.apply_building_layout(text, text, jsonb, uuid) TO service_role;

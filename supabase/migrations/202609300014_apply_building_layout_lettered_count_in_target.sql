-- ==========================================================
-- apply_building_layout: harfli xona qavat MAQSADIGA kiradi
-- ==========================================================
-- 202609300013 harfli xonalarni "qo'shimcha" deb qarardi (maqsad = sof
-- raqamli xonalar soni). Lekin user modeli boshqacha: qavatga "9 ta xona"
-- deb belgilanса, o'sha 9 ichida "3a", "7a" ham bor — ya'ni 7 ta raqamli
-- + 2 ta harfli = 9. Va ketma-ket raqamlash haqiqатда qo'yilgan raqamli
-- xonalar bo'yicha oqadi: 1-qavat "1..7 + 3a,7a" bo'lса, 2-qavat "8" dan.
--
-- Shu sababли har qavat uchun:
--   num_target = GREATEST(plan.rooms - <o'sha qavatдаги harfli xonalar>, 0)
-- va sekvensiya ofseti num_target lar yig'indisiдан olinади.
--
-- Imzo o'zgarmadi — CREATE OR REPLACE. 202609300013 ni almashtiradi
-- (ikkalasи ham bir xil funksiyani REPLACE qiladi — birini yoki ikkalasini
-- ketma-ket ishga tushirsangiz oxirgi holat shu).
...
BEGIN
  ...
  FROM (
    ...
    SELECT floor, rooms, num_target,
           CASE WHEN p_numbering = 'per-floor'
             THEN floor * 100 + 1
             ELSE COALESCE(sum(num_target) OVER (ORDER BY floor

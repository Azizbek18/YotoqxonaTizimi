-- Enforce single-gender rooms at the table boundary, not only in the room
-- assignment RPCs.  This covers registration, admin/service-role writes and
-- future code paths which write users/permit_requests directly.
...
DECLARE
  ...
  v_lock_key text;
BEGIN
  -- Only residents and approved room reservations occupy a room.
  IF TG_TABLE_NAME = 'users' AND (NEW.role IS DISTINCT FROM 'talaba' OR NEW.room_number IS NULL) THEN
    RETURN NEW;
  END IF;
  IF TG_TABLE_NAME = 'permit_requests' AND (NEW.status IS DISTINCT FROM 'approved' OR NEW.room_number IS NULL) THEN
    RETURN NEW;
  END IF;
...
DROP TRIGGER IF EXISTS enforce_users_room_gender ON public.users;
CREATE TRIGGER enforce_users_room_gender
BEFORE INSERT OR UPDATE OF role, status, gender, room_number, dorm_id, block, assigned_floor
ON public.users
FOR EACH ROW EXECUTE FUNCTION public.enforce_room_gender_integrity();

DROP TRIGGER IF EXISTS enforce_permit_room_gender ON public.permit_requests;
CREATE TRIGGER enforce_permit_room_gender
BEFORE INSERT OR UPDATE OF status, gender, room_number, dorm_id, block, assigned_floor
